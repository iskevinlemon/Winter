...
(function (Factory) { // function attached to a window object - START
  ...
  */
  ...
  // Click directive with w-click
  Factory.$clickDirective = function (input) {
    return input.replace(/w-click(?=(?:(?:[^"]*"){2})*[^"]*$)(?=(?:(?:[^']*'){2})*[^']*$)|(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/g, 
    (match, comment) => {
      return comment ? match : 'onclick';
    });
  }

  // Disabled directive with w-block
  Factory.$disabledDirective = function (input) {
    return input.replace(/w-block(?=(?:(?:[^"]*"){2})*[^"]*$)(?=(?:(?:[^']*'){2})*[^']*$)|(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/g, 
    (match, comment) => {
      return comment ? match : 'disabled';
    });
  }

  // Onsubmit directive with w-submit
  Factory.$onSubmitDirective = function (input) {
    return input.replace(/w-submit(?=(?:(?:[^"]*"){2})*[^"]*$)(?=(?:(?:[^']*'){2})*[^']*$)|(\/\/[^\n]*|\/\*[\s\S]*?\*\/)/g, 
    (match, comment) => {
  ...
  // Function to get props from custom element
  /*
    <!-- username prop -->
    <my-component username="John">
    </my-component>

    <script>
      // Get the username prop
      var username = document.querySelector("[username]").getAttribute('username');
      console.log(username);
    </script>
  */
 // document.querySelector("[username]").getAttribute('username');
  Factory.$getProp = function (prop){
    return document.querySelector(`[${prop}]`).getAttribute(`${prop}`);
  }
